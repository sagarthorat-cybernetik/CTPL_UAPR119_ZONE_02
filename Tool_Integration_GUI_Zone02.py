...
class GUI_load(QMainWindow):
    def __init__(self):
        ...
        if station_no in web_station:
            ...
            ui_file = os.path.join(self.root_Path, "Station_GUI_livgaurd_Zone_02_Station_05.ui")
        else:
            ui_file = os.path.join(self.root_Path,"Station_GUI_livgaurd_Zone_02_all.ui")

        loadUi(ui_file, self)
        self.loggedinuser = "demo"
        self.shift = "demo"
        ...
        if station_no =="05":
            # PLC  and meter connection class call pass ip and port
            meter_host = "192.168.205.131"
            meter_port = 23
            # PLC address configurations
            plc_host = "192.168.205.81"
            plc_port = 502

            # start the PLC and METER in the new thread
            # self.plc_meter = PlcMeter(meter_host, meter_port, plc_host, plc_port)
            # self.thread = threading.Thread(target=self.plc_meter.operate, daemon=True)
            # self.thread.start()
            #
            '''Log data'''
            self.log_data = Log_data(self)
    ...
    def login(self):
        try:
            ...
            if self.user_passwords.get(selected_user) == entered_password:
                #     open messagebox and ask for the name
                name, ok = QtWidgets.QInputDialog.getText(self, "Operator Login", "Please Enter Your Name:")
    ...
    def Load_Webpage(self, staion_no="01"):
        try:
            if not hasattr(self,'SOP_img_lbl') or not isinstance(self.Webpage,QLabel):
                print("SOP lable is not QLable")
                return
            if not hasattr(self, 'web_view'):
                ...
                print("website already exit no need to recreate!!")
        except Exception as e:
            print(f"Error loading Website inside QLable: {e}")
    ...
    def copy_files(self):
        ...
        source_folder = os.path.expanduser("~/Desktop/Cognex")  # Change if needed

        #  Destination base folder (where the script runs)
        destination_base = os.getcwd()  # Current working directory
        #  Backup Folder (where images will be backed up)
        backup_folder = os.path.expanduser("~/Desktop/Cognex_backup")  # Change if needed
        os.makedirs(backup_folder, exist_ok=True)  # Create if not exists

        #  Get current date
        today = datetime.today()
        year_folder = str(today.year)  # 2025
        month_folder = today.strftime("%B")  # "March"
        day_folder = today.strftime("%d")  # "03"

        #  Final destination path (2025/March/03/)
        destination_folder = os.path.join(destination_base, year_folder, month_folder, day_folder)
        os.makedirs(destination_folder, exist_ok=True)  # Create if not exists

        #  Allowed file extensions (including SVG)
        allowed_extensions = (".jpg", ".png", ".jpeg", ".bmp", ".gif", ".webp", ".svg")

        # Copy only new images (including SVG)
        if os.path.exists(source_folder):
            for file in os.listdir(source_folder):
                if file.lower().endswith(allowed_extensions):  # Image + SVG filter
                    source_path = os.path.join(source_folder, file)
                    destination_path = os.path.join(destination_folder, file)
                    backup_file_path = os.path.join(backup_folder, file)
                    # Only copy if the file does NOT already exist in the destination
                    if not os.path.exists(destination_path):
                        shutil.copy2(source_path, destination_path)  # Copy with metadata
                        shutil.copy2(source_path, backup_file_path)  # Copy with metadata
                        os.remove(source_path)

        self.modbus_worker.client.write_single_register(280, 0)
